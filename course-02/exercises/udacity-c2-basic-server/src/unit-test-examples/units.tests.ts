...
describe('concat', () => {
  it('should successfully add 2 strings together', () => {
    const result = concat('sea','horse');
    expect(result).to.equal('seahorse');
  })

  it('should throw an error if empty string', () => {
    expect(()=>{ concat('','ok') }).to.throw('There\'s an empty string')
  });
});
